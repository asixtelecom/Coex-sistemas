...
trait Google_Trait {
    ...
    public function authorize() {
        validate_numeric_value($this->mailbox_id);
    ...
    private function _check_access_token($client, $redirect_to_settings = false) {
        ...
        $accessToken = get_mailbox_setting("mailbox_" . $this->mailbox_id . "_gmail_" . $this->type . "_oauth_access_token");
    ...
    public function save_access_token($auth_code) {
        ...
        if ($new_access_token) {
            ...
            if ($this->type == "imap") {
                ...
                $this->Mailbox_settings_model->save_setting("mailbox_" . $this->mailbox_id . "_gmail_" . $this->type . "_email", $google_account_info->email);
            } else {
            ...
            $this->Mailbox_settings_model->save_setting("mailbox_" . $this->mailbox_id . "_gmail_" . $this->type . "_oauth_access_token", $new_access_token);
    ...
    private function _get_client_credentials() {
        ...
        $url = get_uri("mailbox_gmail_api/save_gmail_" . $this->type . "_access_token/" . $this->mailbox_id);

        $client = new \Google_Client();
        $client->setApplicationName(get_setting('app_title'));
        $client->setRedirectUri($url);
        $client->setClientId(get_mailbox_setting("mailbox_" . $this->mailbox_id . "_gmail_" . $this->type . "_client_id"));
        $client->setClientSecret(get_mailbox_setting("mailbox_" . $this->mailbox_id . "_gmail_" . $this->type . "_client_secret"));
