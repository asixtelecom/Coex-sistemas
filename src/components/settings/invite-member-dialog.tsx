...
export function InviteMemberDialog({
  ...
  async function handleCreate() {
    ...
    if (trimmedLabel.length > MAX_LABEL_LEN) {
      toast.error(`O identificador deve ter no máximo ${MAX_LABEL_LEN} caracteres`);
      return;
    }
    setSubmitting(true);
    try {
      ...
      if (!res.ok) {
        ...
        toast.error(payload.error || 'Erro ao criar convite');
        return;
      }
      ...
      toast.error('Não foi possível contactar o servidor. Tentar novamente?');
    } finally {
      setSubmitting(false);
    }
  }
  ...
  function whatsappShareUrl(url: string): string {
    ...
    const message = `Entre no ${accountName} no wacrm usando este link (válido por ${result?.expiresInDays} dias): ${url}`;
    ...
    >
      <DialogContent className="bg-popover border-border sm:max-w-md">
        {result ? (
          <>
            ...
            <div className="space-y-3 py-2">
              ...
              <div className="rounded-md border border-amber-500/50 bg-amber-500/15 px-3 py-2 text-xs text-amber-200">
                ...
                Nunca armazenamos o texto original — ao fechar esta janela
                a URL desaparece. Para reenviar, revogue este convite e crie
                um novo.
              </div>
          ...
          <>
            <DialogHeader>
              <DialogTitle className="text-popover-foreground">Convidar um membro</DialogTitle>
              <DialogDescription className="text-muted-foreground">
                Gere um link de convite único. Compartilhe via WhatsApp,
                Slack ou qualquer canal — não precisa de e-mail.
              </DialogDescription>
            </DialogHeader>
